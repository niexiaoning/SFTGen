"""
ArborGraph-Intent (Domain-Agnostic Tree-of-Graphs) API endpoints.

Provides endpoints for:
- ArborGraph-Intent configuration management
- Taxonomy tree management
- ArborGraph-Intent pipeline execution
"""
...
class IntentConfig(BaseModel):
    """ArborGraph-Intent configuration request/response model."""
...
class IntentPipelineConfig(BaseModel):
    """ArborGraph-Intent pipeline execution request."""
...
class SaveIntentConfigRequest(BaseModel):
    """Save ArborGraph-Intent configuration request."""
...
class RunIntentPipelineRequest(BaseModel):
    """Run ArborGraph-Intent pipeline request."""
...
# ─── ArborGraph-Intent Configuration Endpoints ─────────────────────────────


@router.post("/intent/config/save")
async def save_intent_config(
    request: SaveIntentConfigRequest,
    ...
    config_file = user_config_dir / f"{current_user.username}_intent.json"
...
@router.post("/intent/config/load")
async def load_intent_config(
    current_user: User = Depends(get_current_user),
):
    """加载 ArborGraph-Intent 配置"""
    # Load user's ArborGraph-Intent config from file
    user_config_dir = PROJECT_ROOT / "cache/user_config"
    config_file = user_config_dir / f"{current_user.username}_intent.json"
...
# ─── ArborGraph-Intent Taxonomy Endpoints ─────────────────────────────


@router.post("/intent/taxonomy/save")
async def save_taxonomy_tree(
    data: IntentConfig,
    current_user: User = Depends(require_admin),
):
    """保存 ArborGraph-Intent 意图树"""
...
@router.get("/intent/taxonomy/list")
async def list_taxonomy_trees(
    current_user: User = Depends(get_current_user),
):
    """获取用户的所有 ArborGraph-Intent 意图树"""
    user_taxonomies_dir = PROJECT_ROOT / "data/taxonomies" / current_user.username
...
@router.get("/intent/taxonomy/{taxonomy_id}")
async def get_taxonomy_tree(
    taxonomy_id: str,
    current_user: User = Depends(get_current_user),
):
    """获取单个 ArborGraph-Intent 意图树详情"""
...
@router.put("/intent/taxonomy/{taxonomy_id}")
async def update_taxonomy_tree(
    taxonomy_id: str,
    data: IntentConfig,
    current_user: User = Depends(require_admin),
):
    """更新 ArborGraph-Intent 意图树"""
...
@router.delete("/intent/taxonomy/{taxonomy_id}")
async def delete_taxonomy_tree(
    taxonomy_id: str,
    current_user: User = Depends(require_admin),
):
    """删除 ArborGraph-Intent 意图树"""
...
# ─── ArborGraph-Intent Pipeline Endpoints ─────────────────────────────


@router.post("/intent/pipeline/run")
async def run_intent_pipeline(
    request: IntentPipelineConfig,
    current_user: User = Depends(get_current_user),
):
    """运行 ArborGraph-Intent 管道生成数据"""
    import asyncio

    # For now, this is a placeholder - actual implementation
    # should call the arborgraph intent CLI via subprocess
    # or integrate IntentPipeline into the web service

    # Simulate creating a task
    result = {
        "task_id": f"intent_{int(asyncio.get_event_loop().time())}",
        "status": "running",
    ...
    # Return the result
    return JSONResponse(content={"success": True, "data": result, "message": "ArborGraph-Intent 管道已启动"})


@router.get("/intent/pipeline/status/{task_id}")
async def get_pipeline_status(
    task_id: str,
    current_user: User = Depends(get_current_user),
):
    """获取 ArborGraph-Intent 管道运行状态"""
    # For now, return placeholder status
    return JSONResponse(content={
        "success": True,
        "task_id": task_id,
        "status": "not_started",
        "message": "ArborGraph-Intent 管道功能开发中"
    })
