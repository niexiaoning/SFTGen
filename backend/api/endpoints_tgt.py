"""
TGT (Domain-Agnostic Tree-of-Graphs) API endpoints.

Provides endpoints for:
- TGT configuration management
- Taxonomy tree management
- TGT pipeline execution
"""
...
class TGTConfig(BaseModel):
    """TGT configuration request/response model."""
...
class TGTPipelineConfig(BaseModel):
    """TGT pipeline execution request."""
...
class SaveTGTConfigRequest(BaseModel):
    """Save TGT configuration request."""
...
class RunTGTPipelineRequest(BaseModel):
    """Run TGT pipeline request."""
...
# ─── TGT Configuration Endpoints ─────────────────────────────


@router.post("/tgt/config/save")
async def save_tgt_config(
    request: SaveTGTConfigRequest,
    ...
    config_file = user_config_dir / f"{current_user.username}_tgt.json"
...
async def load_tgt_config(
    ...
    """加载 TGT 配置"""
    # Load user's TGT config from file
    user_config_dir = PROJECT_ROOT / "cache/user_config"
    config_file = user_config_dir / f"{current_user.username}_tgt.json"
...
# ─── TGT Taxonomy Endpoints ─────────────────────────────


@router.post("/tgt/taxonomy/save")
...
@router.get("/tgt/taxonomy/{taxonomy_id}")
...
@router.put("/tgt/taxonomy/{taxonomy_id}")
...
@router.delete("/tgt/taxonomy/{taxonomy_id}")
...
# ─── TGT Pipeline Endpoints ─────────────────────────────


@router.post("/tgt/pipeline/run")
async def run_tgt_pipeline(
    request: TGTPipelineConfig,
    current_user: User = Depends(get_current_user),
):
    """运行 TGT 管道生成数据"""
    import asyncio
    ...
    # should call the graphgen tgt CLI via subprocess
    # or integrate TGTPipeline into the web service

    # Simulate creating a task
    result = {
        "task_id": f"tgt_{int(asyncio.get_event_loop().time())}",
        "status": "running",
    ...
    # Return the result
    return JSONResponse(content={"success": True, "data": result, "message": "TGT 管道已启动"})


@router.get("/tgt/pipeline/status/{task_id}")
