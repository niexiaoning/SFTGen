...
**聚合式 QA 生成**（`Aggregation-basedGenerator`）：先将子图中多个实体和关系重述为连贯段落作为答案，再反向生成问题。支持合并模式一次性生成，减少 50% API 调用。
...
**合并生成模式**：Aggregation-based、CoT 等生成器支持合并模式，将两阶段生成压缩为单次调用，减少 50% API 调用。解析失败时自动回退到两阶段模式。
