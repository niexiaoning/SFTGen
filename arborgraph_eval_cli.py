...
def main():
    ...
    parser.add_argument(
        ...
        default=files("arborgraph").joinpath("configs", "evaluation_config.yaml"),
        type=str,
    )
    parser.add_argument(
        "--output_dir",
    ...
    # ArborGraph-Intent 指标参数
    intent_group = parser.add_argument_group("ArborGraph-Intent 指标计算")
    intent_group.add_argument("--intent-taxonomy", help="ArborGraph-Intent 意图树文件路径", required=False)
    intent_group.add_argument("--intent-results", help="ArborGraph-Intent 结果文件路径", required=False)
    intent_group.add_argument("--intent-output", help="ArborGraph-Intent 指标输出文件路径", required=False)
    ...
    arbor_graph.generate_evaluation(
        partition_config=config["partition"],
        evaluation_config=config["evaluation"],
    )

    # ArborGraph-Intent 指标计算模式
    if hasattr(args, 'intent_taxonomy'):
        success = run_intent_metrics(args)
