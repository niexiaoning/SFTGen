"""
DA-ToG Generator for the GraphGen pipeline.
...
class DAToGGenerator(BaseGenerator):
    """
    ...
    def build_prompt(
        ...
        """
        Build a DA-ToG prompt.

        Can work in two modes:
        1. Standard mode (batch only): falls back to basic serialization
        2. Intent-aware mode (batch + intent + subgraph_text): full DA-ToG prompting
        """
    ...
    def parse_response(response: str) -> dict:
        ...
        if question and answer:
            key = f"datog_{hash(question) % 10000:04d}"
            result[key] = {
                "question": question,
                "answer": answer,
                "mode": "datog",
