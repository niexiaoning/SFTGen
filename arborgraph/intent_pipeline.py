"""
ArborGraph-Intent Main Pipeline.

Assembles the three ArborGraph-Intent layers:
...
from arborgraph.models.generator.intent_generator import IntentGenerator
...
class IntentPipeline:
    ...
    def __init__(
        ...
        self.generator = IntentGenerator(llm_client)
    ...
    def from_config(
        ...
        """
        Create a ArborGraph-Intent pipeline from a YAML config file.

        :param config_path: Path to intent_config.yaml
        ...
        intent_config = config.get("intent", config)
        ...
        if taxonomy_path and os.path.exists(taxonomy_path):
            ...
            raise ValueError(
                ...
                "Set intent.taxonomy.path in config."
            )

        # Create graph adapter
        graph_config = intent_config.get("graph", {})
        ...
        # Create critic
        critic_config = intent_config.get("critic", {})
    ...
    async def run(
        ...
        """
        Execute the full ArborGraph-Intent synthesis pipeline.
        ...
        """
        logger.info(
            "ArborGraph-Intent pipeline starting: target=%d, strategy=%s, tree=%s",
        ...
        logger.info(
            "ArborGraph-Intent pipeline complete: generated %d/%d QA pairs, "
            "coverage=%.1f%%",
    ...
    async def _generate_for_intent(
        ...
        # Step 2b: Generate using IntentGenerator
