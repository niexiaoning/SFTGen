"""
ArborGraph-Intent Generator for the ArborGraph pipeline.
...
class IntentGenerator(BaseGenerator):
    ...
    def build_prompt(
        ...
        """
        Build a ArborGraph-Intent prompt.

        Can work in two modes:
        1. Standard mode (batch only): falls back to basic serialization
        2. Intent-aware mode (batch + intent + subgraph_text): full ArborGraph-Intent prompting
        """
    ...
    def parse_response(response: str) -> dict:
        ...
        if question and answer:
            key = f"intent_{hash(question) % 10000:04d}"
            result[key] = {
                "question": question,
                "answer": answer,
                "mode": "intent",
    ...
    async def generate_with_intent(
        ...
        # Add ArborGraph-Intent metadata to each pair
