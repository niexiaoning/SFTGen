...
from arborgraph.bases import BaseLLMClient
...
from arborgraph.models.llm.batch_llm_wrapper import BatchLLMWrapper
from arborgraph.templates import ATOMIC_ANSWER_PROMPT
from arborgraph.utils import compute_content_hash, detect_main_language, logger, run_concurrent
...
async def generate_qas(
    ...
    if mode == "all":
        ...
        generators = [
            ...
            (IntentGenerator(actual_llm_client, data_format=data_format, default_dimension=generation_config.get("default_dimension", "concept_explanation")), "intent"),
        ]
        ...
        async def run_atomic_two_stage() -> list[dict[str, Any]]:
            ...
            async def answer_question(entry: dict[str, Any]) -> dict[str, Any]:
                try:
                    ...
                    if chinese_only:
                        from arborgraph.templates import ATOMIC_ANSWER_PROMPT_CHINESE_ONLY
