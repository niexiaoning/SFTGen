# DA-ToG: 基于现有 SFTGen 代码库的分步实施计划

## 问题背景

[optimization.md](./optimization.md) 提出了 **DA-ToG (Domain-Agnostic Tree-of-Graphs Data Synthesis)** 框架，其核心思想是将 Condor（树状知识先验）和 GraphGen（知识图谱引导）两种方法统一为一个"领域无关"的合成数据引擎。该框架包含三个可插拔层：
...
**核心差距**：当前系统是"一体化"的，缺乏"可插拔领域配置"机制。DA-ToG 要求用户仅通过替换"配置包"即可适配新领域。
...
| | DAToGPipeline | ✅ 完成 | `graphgen/datog_pipeline.py` |
| **Phase 4** | DAToGGenerator | ✅ 完成 | `graphgen/models/generator/datog_generator.py` |
| | 认知维度模板 | ✅ 完成 | 内置 6 种认知维度模板 |
| **Phase 5** | 领域配置包 | ✅ 完成 | `graphgen/configs/datog/finance/` 和 `cybersecurity/` |
| | DAToGMetrics | ✅ 完成 | `graphgen/utils/datog_metrics.py` |
| | 测试文件 | ✅ 完成 | 6 个测试文件 |
| | Generator 集成 | ✅ 完成 | `generate_qas.py` 中已添加 datog 模式 |

### ❌ 待完成组件

| Phase | 组件 | 状态 | 说明 |
|--------|--------|------|------|
| **Phase 3** | CLI `datog` 子命令 | ❌ 缺失 | 需要在 `graphgen_cli.py` 中添加 |
| **Phase 6** | CLI `datog-metrics` 子命令 | ❌ 缺失 | 需要在 `graphgen_eval_cli.py` 中添加 |
| **Phase 5** | `run_domain.py` 脚本 | ❌ 缺失 | 领域实例化批量运行脚本 |
| Bug | `datog_metrics.py` 修复 | ❌ 缺失 | `_get_all_dimensions()` 方法不存在 |
...
## Phase 1: 修复 DAToGMetrics 中的 Bug

**目标**：修复 `datog_metrics.py` 中引用的不存在方法。
...
| 测试项 | 方法 | 命令 | 预期结果 |
|--------|------|------|----------|
| Bug 修复验证 | 单元测试 | `python -m pytest tests/test_datog_metrics.py -v` | 通过，无 AttributeError |

---

## Phase 2: 添加 CLI `datog` 子命令

**目标**：在 `graphgen_cli.py` 中添加 DA-ToG 模式的独立命令，提供与 `DAToGPipeline.from_config()` 的集成。

### 修改文件

#### [MODIFY] `graphgen_cli.py`

在 CLI 主函数中添加 `datog` 子命令：

```python
@cli.command()
@click.option("--config", "-c", type=click.Path(exists=True), required=True,
              help="Path to datog_config.yaml")
...
async def datog(config: str, output: str, input: str, count: int):
    """
    Run DA-ToG (Domain-Agnostic Tree-of-Graphs) pipeline.
    ...
    pipeline = DAToGPipeline.from_config(config, llm_client, graph_storage)

    logger.info(f"Running DA-ToG pipeline, target: {count} QA pairs")
    results = await pipeline.run(target_count=count, batch_size=datog_config.get("generation", {}).get("batch_size", 10))

    # Write output
    if not output:
        output = datog_config.get("generation", {}).get("output", "datog_output.json")
...
| 测试项 | 方法 | 命令 | 预期结果 |
|--------|------|------|----------|
| CLI 帮助 | 手动测试 | `python graphgen_cli.py datog --help` | 显示正确的帮助信息 |
| 运行完整流程 | 集成测试 | 使用配置文件运行 | 生成 QA 对并保存到输出文件 |

---

## Phase 3: 添加 CLI `datog-metrics` 子命令

**目标**：在 `graphgen_eval_cli.py` 中添加 DA-ToG 特定的评测命令。

### 修改文件

#### [MODIFY] `graphgen_eval_cli.py`

添加 `datog-metrics` 子命令：

```python
@eval_cli.group()
def datog_metrics():
    """DA-ToG specific evaluation commands."""

@datog_metrics.command()
...
def calculate_datog_metrics(taxonomy: str, results: str, output: str):
    """
    Calculate DA-ToG coverage and distribution metrics.
...
| 测试项 | 方法 | 命令 | 预期结果 |
|--------|------|------|----------|
| CLI 帮助 | 手动测试 | `python graphgen_eval_cli.py datog-metrics --help` | 显示正确的帮助信息 |
...
**目标**：创建 `scripts/run_domain.py` 脚本，支持批量运行 DA-ToG 用于不同领域。
...
async def run_domain(
    ...
    taxonomy_path = datog_config.get("taxonomy", {}).get("path")
    ...
    logger.info("Running DA-ToG pipeline...")
...
```mermaid
graph TD
    P1["Phase 1<br/>修复 datog_metrics Bug"] --> P2
    P2["Phase 2<br/>CLI datog 子命令"] --> P4
    P3["Phase 3<br/>CLI datog-metrics 子命令"] --> P4
...
# 运行 DA-ToG 相关测试
python -m pytest tests/test_taxonomy_tree.py tests/test_graph_adapter.py tests/test_critic.py tests/test_datog_pipeline.py tests/test_datog_generator.py tests/test_datog_metrics.py -v
```
...
python scripts/run_domain.py --domain graphgen/configs/datog/finance --output output/finance

# 使用 datog-metrics 评测输出
python graphgen_eval_cli.py datog-metrics --taxonomy graphgen/configs/datog/finance/taxonomy.json --results output/finance/qa_pairs.json
```
...
```bash
# 确认现有功能不受影响
python graphgen_cli.py --config graphgen/configs/aggregated_config.yaml --input resources/input_examples/jsonl_demo.jsonl --dry-run
```
...
| 组件 | 类/函数 | 位置 |
|--------|----------|------|
| DA-ToG 管道 | `DAToGPipeline` | `graphgen/datog_pipeline.py` |
| DA-ToG 生成器 | `DAToGGenerator` | `graphgen/models/generator/datog_generator.py` |
...
| 组件 | 类/函数 | 位置 |
|--------|----------|------|
| 评估指标 | `DAToGMetrics` | `graphgen/utils/datog_metrics.py` |
| 配置模板 | `datog_config.yaml` | `graphgen/configs/datog_config.yaml` |
| 金融配置 | `taxonomy.json`, `datog_config.yaml` | `graphgen/configs/datog/finance/` |
| 安全配置 | `taxonomy.json`, `datog_config.yaml` | `graphgen/configs/datog/cybersecurity/` |
