<template>
  <div class="tgt-config-container">
    <el-card>
      <template #header>
      ...
      <el-form :model="config" label-width="180px" label-position="left">
        <el-collapse v-model="activeNames">
          ...
          <el-collapse-item title="意图树配置" name="taxonomy">
            <el-alert
              title="TGT (Domain-Agnostic Tree-of-Graphs)"
              type="info"
              :closable="false"
            ...
            >
              <template #default>
                <div style="font-size: 12px; line-height: 1.6">
                  <p>TGT 是一个领域无关的树状图生成框架，通过三层次结构实现高质量SFT数据生成：</p>
...
const config = ref({ ...tgtStore.config })
...
const handleSave = async () => {
  saving.value = true
  try {
    tgtStore.config = { ...config.value }
    const success = await tgtStore.saveConfig()
    if (success) {
      config.value = { ...tgtStore.config }
    }
  } finally {
    saving.value = false
  }
...
onMounted(async () => {
  await tgtStore.loadConfig()
  config.value = { ...tgtStore.config }
})
</script>

<style scoped>
.tgt-config-container {
  max-width: 1200px;
  margin: 0 auto;
