<template>
  <div class="intent-pipeline-container">
    <el-card>
      <template #header>
        <div class="card-header">
          <span>ArborGraph-Intent 管道</span>
        </div>
      </template>

      <el-alert
        title="ArborGraph-Intent 管道说明"
        type="info"
        :closable="false"
      ...
      >
        <template #default>
          <div style="font-size: 13px; line-height: 1.8">
            <p>ArborGraph-Intent 管道通过以下步骤生成领域特定的SFT数据：</p>
          ...
          <el-collapse-item title="基础配置" name="basic">
            <el-form-item label="领域配置路径">
              <el-input
                ...
                placeholder="arborgraph/configs/intent/xxx"
              />
          ...
          <el-collapse-item title="输出配置" name="output">
            <el-form-item label="输出路径">
              <el-input
                ...
                placeholder="data/output/intent/xxx.json"
              />
...
import type { IntentPipelineConfig, IntentTaxonomy, PipelineTaskStatus } from '@/api/types'
...
const pipelineConfig = ref<IntentPipelineConfig>({
  domain_config_path: '',
  taxonomy_path: '',
  input_file: '',
  kg_path: '',
...
const availableTaxonomies = ref<IntentTaxonomy[]>([])

const pipelineStatus = computed(() => intentStore.pipelineStatus)
const running = computed(() => intentStore.pipelineRunning)
...
const loadAvailableTaxonomies = async () => {
  ...
  try {
    await intentStore.loadTaxonomies()
    availableTaxonomies.value = intentStore.taxonomies
  } finally {
...
// 选择意图树
const selectTaxonomy = (taxonomy: IntentTaxonomy) => {
...
const handleRun = async () => {
  ...
  try {
    await ElMessageBox.confirm(
      '确定要启动 ArborGraph-Intent 管道吗？这可能需要较长时间。',
      '确认启动',
      {
    ...
    const result = await intentStore.runPipeline(pipelineConfig.value)
    if (result) {
...
const refreshStatus = async () => {
  if (pipelineStatus.value?.task_id) {
    await intentStore.getPipelineStatus(pipelineStatus.value.task_id)
  }
}

// 清除状态
const clearStatus = () => {
  intentStore.pipelineStatus = null
  stopAutoRefresh()
}
