    ...
    <el-card>
      <template #header>
        <div class="card-header">
          <span>ArborGraph-Intent 配置</span>
      ...
      <el-form :model="config" label-width="180px" label-position="left">
        <el-collapse v-model="activeNames">
          ...
          <el-collapse-item title="意图树配置" name="taxonomy">
            <el-alert
              title="ArborGraph-Intent (Domain-Agnostic Tree-of-Graphs)"
              type="info"
              :closable="false"
            ...
            >
              <template #default>
                <div style="font-size: 12px; line-height: 1.6">
                  <p>ArborGraph-Intent 是一个领域无关的树状图生成框架，通过三层次结构实现高质量SFT数据生成：</p>
...
const config = ref({ ...intentStore.config })
...
const handleSave = async () => {
  saving.value = true
  try {
    intentStore.config = { ...config.value }
    const success = await intentStore.saveConfig()
    if (success) {
      config.value = { ...intentStore.config }
    }
  } finally {
    saving.value = false
  }
