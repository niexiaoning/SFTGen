<template>
  <div class="intent-taxonomies-container">
    <el-card>
      <template #header>
    ...
    >
      <el-form :model="formData" label-width="120px">
        ...
        <el-divider>ArborGraph-Intent 配置</el-divider>
...
import type { IntentConfig, IntentTaxonomy, TaxonomyNode, TaxonomyStatistics } from '@/api/types'
...
const selectedTaxonomy = ref<IntentConfig | null>(null)
...
const formData = ref<IntentConfig>({
  taxonomy_path: '',
  domain: '',
...
const loadTaxonomies = async () => {
  ...
  try {
    ...
    taxonomies.value = intentStore.taxonomies
  } finally {
    loading.value = false
  }
}
...
// 查看意图树
const handleView = async (taxonomy: IntentTaxonomy) => {
  const data = await intentStore.getTaxonomy(taxonomy.id)
  if (data) {
...
// 编辑意图树
const handleEdit = async (taxonomy: IntentTaxonomy) => {
  const data = await intentStore.getTaxonomy(taxonomy.id)
  if (data) {
...
// 删除意图树
const handleDelete = async (taxonomy: IntentTaxonomy) => {
  try {
    ...
    await intentStore.deleteTaxonomy(taxonomy.id)
  } catch (error) {
    // 用户取消
  }
}
...
const handleSave = async () => {
  ...
  try {
    if (isEditMode.value) {
      await intentStore.updateTaxonomy(editingTaxonomyId.value, formData.value)
    } else {
      await intentStore.saveTaxonomy(formData.value)
    }
