<template>
  <div class="tgt-taxonomies-container">
    <el-card>
      <template #header>
...
import type { TGTConfig, TGTTaxonomy, TaxonomyNode, TaxonomyStatistics } from '@/api/types'
...
const selectedTaxonomy = ref<TGTConfig | null>(null)
...
const loadTaxonomies = async () => {
  ...
  try {
    ...
    taxonomies.value = tgtStore.taxonomies
  } finally {
    loading.value = false
  }
}
...
// 查看意图树
const handleView = async (taxonomy: TGTTaxonomy) => {
  const data = await tgtStore.getTaxonomy(taxonomy.id)
  if (data) {
...
// 编辑意图树
const handleEdit = async (taxonomy: TGTTaxonomy) => {
  const data = await tgtStore.getTaxonomy(taxonomy.id)
  if (data) {
...
// 删除意图树
const handleDelete = async (taxonomy: TGTTaxonomy) => {
  try {
    ...
    await tgtStore.deleteTaxonomy(taxonomy.id)
  } catch (error) {
    // 用户取消
  }
}
...
const handleSave = async () => {
  ...
  try {
    if (isEditMode.value) {
      await tgtStore.updateTaxonomy(editingTaxonomyId.value, formData.value)
    } else {
      await tgtStore.saveTaxonomy(formData.value)
    }
