"""
Tests for TGT Phase 4: Meta-Prompt Template and TGTGenerator.
"""
...
from textgraphtree.models.generator.tgt_generator import TGTGenerator, DIMENSION_PROMPTS


class TestTGTGenerator:
    def test_build_intent_prompt(self):
        mock_llm = AsyncMock()
        generator = TGTGenerator(mock_llm)
    ...
    def test_generate_with_intent(self):
        async def _run():
            ...
            assert len(results) == 1
            key = list(results.keys())[0]
            assert results[key]["metadata"]["generation_mode"] == "tgt"
        
        asyncio.run(_run())
        
    def test_all_dimensions_have_prompts(self):
        from textgraphtree.models.taxonomy.taxonomy_tree import COGNITIVE_DIMENSIONS
