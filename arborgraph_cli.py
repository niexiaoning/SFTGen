...
"""
ArborGraph 命令行工具
将 ArborGraph Demo web app 转换为命令行脚本版本
"""
...
class ArborGraphCLI:
    ...
    def init_arbor_graph(self, config: dict, env: dict) -> ArborGraph:
        """初始化 ArborGraph 实例"""
        # 设置工作目录
        ...
        # 创建 ArborGraph 实例（传递 synthesizer_llm_client 和 trainee_llm_client）
    ...
    def setup_batch_logging(self, log_file: str):
        ...
        self.batch_logger.info("ArborGraph 批量处理开始")
    ...
    def process_single_file(self, file_path: str, args, progress_bar=None) -> dict:
        ...
        try:
            ...
            # 初始化 ArborGraph
            arbor_graph = self.init_arbor_graph(config, env)
            arbor_graph.clear()
            ...
            if quiz_and_judge_config["enabled"]:
                arbor_graph.quiz_and_judge(quiz_and_judge_config=quiz_and_judge_config)
            ...
            output_file = f"{base_name}_arborgraph_output.jsonl"
            
            with open(output_file, "w", encoding="utf-8") as f:
                json.dump(output_data, f, ensure_ascii=False, indent=2)

            # 计算实际使用的 token
            synthesizer_tokens = sum(u["total_tokens"] for u in arbor_graph.synthesizer_llm_client.token_usage)
            trainee_tokens = sum(u["total_tokens"] for u in arbor_graph.trainee_llm_client.token_usage) if config["if_trainee_model"] else 0
    ...
    def run_batch_processing(self, args):
        ...
        # 设置批量处理日志
        log_file = f"arborgraph_batch_{datetime.now().strftime('%Y%m%d_%H%M%S')}.log"
    ...
    def run_arborgraph(self, args):
        """运行 ArborGraph 主流程"""
        print("🚀 开始运行 ArborGraph...")
    ...
    def run_intent(self, args):
        ...
        print("🚀 开始运行 ArborGraph-Intent 管道...")

        # Load config
        with open(args.intent_config, "r", encoding="utf-8") as f:
        ...
        if args.input_file:
            ...
            arbor_graph = ArborGraph(working_dir=working_dir, tokenizer_instance=tokenizer_instance, synthesizer_llm_client=llm_client)
            arbor_graph.clear()
            ...
            arbor_graph.insert(read_config=read_config, split_config=split_config)
            graph_storage = arbor_graph.qa_storage
            print("✅ 知识图谱构建完成")
        elif args.kg_path:
        ...
        print("🔧 初始化 ArborGraph-Intent 管道...")
        pipeline = IntentPipeline.from_config(args.intent_config, llm_client, graph_storage)
        ...
        try:
            ...
            if args.output_file:
                ...
                base_name = os.path.splitext(os.path.basename(args.intent_config))[0]
                output_file = f"{base_name}_intent_output.json"
...
def create_parser():
    ...
    parser = argparse.ArgumentParser(
        description="ArborGraph 命令行工具 - 从文本生成知识图谱训练数据",
...
示例用法:
  ...
  # 批量处理多个文件
  python arborgraph_cli.py -b file1.txt file2.json file3.csv -k your_api_key

  # 从文件列表批量处理
  python arborgraph_cli.py -l file_list.txt -k your_api_key

  # 使用 Trainee 模型
  python arborgraph_cli.py -i input.txt -k your_api_key --use-trainee-model --trainee-api-key your_trainee_key

  # 自定义参数
  python arborgraph_cli.py -i input.txt -k your_api_key --chunk-size 2048 --max-depth 3
        """
    )
    ...
    # ArborGraph-Intent 模式参数组
    intent_group = parser.add_argument_group("ArborGraph-Intent 模式")
    intent_group.add_argument("--intent-config", help="ArborGraph-Intent 配置文件路径")
    intent_group.add_argument("--intent-input", help="构建知识图谱的输入文件 (与 --intent-config 配合使用)")
    intent_group.add_argument("--intent-kg", help="现有知识图谱路径")
    intent_group.add_argument("--intent-output", help="ArborGraph-Intent 输出文件路径")
...
def main():
    ...
    # ArborGraph-Intent 模式检查
    if hasattr(args, 'intent_config') and args.intent_config:
        cli = ArborGraphCLI()
        # 设置 intent 参数
        args.intent_input = getattr(args, 'intent_input', None) or args.input_file
        args.intent_kg = getattr(args, 'intent_kg', None)
        args.intent_output = getattr(args, 'intent_output', None) or args.output_file
        success = cli.run_intent(args)
    ...
    if len(input_files) == 1:
        ...
        success = cli.run_arborgraph(args)
    else:
        # 批量处理
