# ArborGraph 代码架构与核心逻辑说明

> 本文档面向新手开发者，详细说明 ArborGraph 项目的主要框架、逻辑和核心模块。
...
ArborGraph 是一个**知识图谱引导的合成数据生成平台**，主要功能是：
...
#### 3. **核心库层** (`graphgen/`)
- **ArborGraph 核心逻辑**：知识图谱构建和 QA 生成的核心算法
...
**代码位置**：`graphgen/graphgen.py::ArborGraph.insert()`
...
**代码位置**：`graphgen/graphgen.py::ArborGraph.quiz_and_judge()`
...
| 文件路径 | 说明 |
|---------|------|
| `graphgen/graphgen.py` | ArborGraph 核心类，包含主要工作流程 |
| `graphgen/generate.py` | CLI 入口，命令行工具的主函数 |
| `backend/app.py` | FastAPI 应用入口 |
| `backend/core/task_processor.py` | 任务处理器，执行 ArborGraph 任务 |
...
3. 执行任务
   └─> backend/core/task_processor.py::process_task()
       └─> 初始化 ArborGraph 实例
           └─> graphgen/graphgen.py::ArborGraph()
...
2. **核心类**：
   - `graphgen/graphgen.py::ArborGraph`：理解主要方法和流程
...
希望这份文档能帮助你快速理解 ArborGraph 的代码架构和核心逻辑！如有疑问，建议查看具体的代码实现和注释。
