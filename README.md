# TextGraphTree 代码架构与核心逻辑说明

> 本文档面向新手开发者，详细说明 TextGraphTree 项目的主要框架、逻辑和核心模块。
...
TextGraphTree 是一个**知识图谱引导的合成数据生成平台**，主要功能是：
...
└─────────────────┼──────────────────────────┼──────────┘
          ...
          └─────────────────┼─────────────────┘
                            │
          ┌─────────────────▼─────────────────┐
          │    TextGraphTree 核心库 (Core Library)  │
          │  ┌──────────────────────────────┐  │
          │  │  知识构建 / 图组织 / QA生成   │  │
          │  │  LLM 调用 / 存储管理          │  │
          │  └──────────────────────────────┘  │
...
#### 1. **前端层** (`frontend/`)
- **Vue 3 前端**：提供现代化的 Web 界面，用于任务管理、数据审核与 TGT 配置
- **CLI 工具**：命令行接口，适合批量处理与离线实验
...
#### 3. **核心库层** (`textgraphtree/`)
- **TextGraphTree 核心逻辑**：知识图谱构建和 QA 生成的核心算法
...
TextGraphTree 的核心工作流程分为**四个主要步骤**：
...
**代码位置**：`textgraphtree/engine.py::TextGraphTree.insert()`

**主要流程**：

1. **文档读取** (`textgraphtree/operators/read/read_files.py`)
   - 支持多种格式：文本、PDF、CSV、JSON、JSONL
   - 使用不同的 Reader 类处理不同格式

2. **文档分割** (`textgraphtree/operators/split/split_chunks.py`)
   - 将长文档分割成较小的 chunk（片段）
   - 支持多种分割策略：递归字符分割、Markdown 分割等
   - 配置参数：`chunk_size`（分块大小）、`chunk_overlap`（重叠大小）

3. **实体和关系提取** (`textgraphtree/operators/build_kg/build_text_kg.py`)
   - 使用 LLM（合成器模型）从每个 chunk 中提取实体和关系
   - 提取器：`textgraphtree/models/kg_builder/light_rag_kg_builder.py`
   - 提示模板：`textgraphtree/templates/kg/kg_extraction.py`
...
```python
# textgraphtree/engine.py:99-194
...
**代码位置**：`textgraphtree/engine.py::TextGraphTree.quiz_and_judge()`

**主要流程**：

1. **语义变体生成** (`textgraphtree/operators/quiz.py`)
   - 对知识图谱中的每条边（关系）生成多个语义变体
   - 生成肯定形式和否定形式
   - 用于后续的置信度评估

2. **置信度评估** (`textgraphtree/operators/judge.py`)
   - 使用训练模型（Trainee Model）对语义变体进行判断
   - 计算模型对每个知识点的置信度
   - 识别知识盲点（置信度低的知识点）

3. **理解损失计算** (`textgraphtree/utils/calculate_confidence.py`)
...
**代码位置**：`textgraphtree/operators/partition/partition_kg.py`
...
```python
# textgraphtree/engine.py:333-360
...
**代码位置**：`textgraphtree/operators/generate/generate_qas.py`
...
2. **问题生成**
   ...
   - 不同模式使用不同的提示模板
   - 模板位置：`textgraphtree/templates/generation/`
...
```python
# textgraphtree/operators/generate/generate_qas.py:89-321
...
**位置**：`textgraphtree/engine.py`

**职责**：TextGraphTree 的核心类，封装了整个工作流程。
...
#### `NetworkXStorage` (`textgraphtree/models/storage/networkx_storage.py`)
- **用途**：存储知识图谱（节点和边）
- **底层**：基于 NetworkX 图库
- **方法**：`add_node()`, `add_edge()`, `get_all_nodes()`, `get_all_edges()`

#### `JsonKVStorage` (`textgraphtree/models/storage/json_storage.py`)
- **用途**：键值对存储（文档、片段、搜索结果等）
- **格式**：JSON 文件
- **方法**：`upsert()`, `get_by_id()`, `filter_keys()`

#### `JsonListStorage` (`textgraphtree/models/storage/json_storage.py`)
- **用途**：列表存储（问答对）
- **格式**：JSON 数组文件
- **方法**：`upsert()`, `all_items()`
...
#### `OpenAIClient` (`textgraphtree/models/llm/openai_client.py`)
...
#### `BatchLLMWrapper` (`textgraphtree/models/llm/batch_llm_wrapper.py`)
...
#### `AtomicGenerator` (`textgraphtree/models/generator/atomic_generator.py`)
- **用途**：生成原子问答对（单个事实）
- **特点**：问题简单直接，答案简短

#### `AggregatedGenerator` (`textgraphtree/models/generator/aggregated_generator.py`) <!-- pragma: allowlist secret -->
- **用途**：生成聚合问答对（多个相关事实）
- **特点**：问题涉及多个实体/关系

#### `MultiHopGenerator` (`textgraphtree/models/generator/multi_hop_generator.py`)
- **用途**：生成多跳问答对（需要推理路径）
- **特点**：问题需要多步推理才能回答

#### `CoTGenerator` (`textgraphtree/models/generator/cot_generator.py`)
- **用途**：生成链式思考问答对
- **特点**：答案包含推理过程

### 5. 分区器（Partitioners）

#### `ECEPartitioner` (`textgraphtree/models/partitioner/ece_partitioner.py`)
- **用途**：基于预期校准误差的分区
- **特点**：优先选择知识盲点（理解损失高的区域）

#### `BFSPartitioner` (`textgraphtree/models/partitioner/bfs_partitioner.py`)
- **用途**：广度优先搜索分区
- **特点**：从根节点开始，逐层扩展

#### `DFSPartitioner` (`textgraphtree/models/partitioner/dfs_partitioner.py`)
- **用途**：深度优先搜索分区
- **特点**：深度优先遍历图

#### `LeidenPartitioner` (`textgraphtree/models/partitioner/leiden_partitioner.py`)
...
- **职责**：处理 TextGraphTree 任务的执行
- **流程**：
  1. 初始化 TextGraphTree 实例
  2. 执行知识构建
  3. 执行理解评估（可选）
  4. 执行 QA 生成
  5. 保存结果并更新任务状态
...
#### `backend/runtime/`
- **职责**：承载任务运行时能力，包括任务状态持久化与工作目录管理
- **说明**：这是重构后替代旧 Gradio `webui` 运行时残留的正式模块
...
│   └── package.json
│
├── textgraphtree/           # TextGraphTree 核心库
│   ├── bases/            # 基础类和接口
│   ├── models/           # 模型实现
│   │   ├── generator/    # 生成器
│   │   ├── partitioner/ # 分区器
...
│   ├── templates/        # 提示模板
│   ├── utils/            # 工具函数
│   ├── engine.py         # TextGraphTree 核心类
│   └── generate.py       # CLI 入口
│
├── backend/runtime/      # 任务运行时与工作目录管理
│
├── docs/                 # 文档
├── scripts/              # 工具脚本
└── resources/            # 资源文件
...
| 文件路径 | 说明 |
|---------|------|
| `textgraphtree/engine.py` | TextGraphTree 核心类，包含主要工作流程 |
| `textgraphtree/generate.py` | CLI 入口，命令行工具的主函数 |
| `backend/app.py` | FastAPI 应用入口 |
| `backend/core/task_processor.py` | 任务处理器，执行 TextGraphTree 任务 |
| `backend/runtime/task_manager.py` | 任务状态持久化与恢复 |
| `textgraphtree/operators/generate/generate_qas.py` | QA 生成的核心逻辑 |
| `textgraphtree/operators/build_kg/build_text_kg.py` | 文本知识图谱构建 |
| `textgraphtree/operators/partition/partition_kg.py` | 图分区逻辑 |
| `textgraphtree/models/generator/` | 各种生成器实现 |
| `textgraphtree/models/partitioner/` | 各种分区器实现 |
...
3. 执行任务
   └─> backend/core/task_processor.py::process_task()
       └─> 初始化 TextGraphTree 实例
           └─> textgraphtree/engine.py::TextGraphTree()

4. 知识构建
   └─> TextGraphTree.insert()
...
5. 理解评估（可选）
   └─> TextGraphTree.quiz_and_judge()
       ├─> 生成语义变体 (operators/quiz.py)
       └─> 评估置信度 (operators/judge.py)

6. QA 生成
   └─> TextGraphTree.generate()
...
1. **入口点**：
   - CLI：`textgraphtree/generate.py::main()`
   - Web API：`backend/main.py` → `backend/app.py` → `backend/core/task_processor.py`

2. **核心类**：
   - `textgraphtree/engine.py::TextGraphTree`：理解主要方法和流程

3. **关键操作**：
   - `textgraphtree/operators/build_kg/build_text_kg.py`：知识构建
   - `textgraphtree/operators/partition/partition_kg.py`：图分区
   - `textgraphtree/operators/generate/generate_qas.py`：QA 生成

4. **生成器**：
   - `textgraphtree/models/generator/atomic_generator.py`：理解生成逻辑
...
**修改生成模式**：
- 编辑 `textgraphtree/models/generator/` 下的生成器类
- 修改提示模板：`textgraphtree/templates/generation/`

**添加新的分区方法**：
- 在 `textgraphtree/models/partitioner/` 下创建新的分区器类
- 实现 `BasePartitioner` 接口
- 在 `textgraphtree/operators/partition/partition_kg.py` 中注册

**修改输出格式**：
- 编辑生成器的 `format_generation_results()` 方法
- 参考：`textgraphtree/models/generator/atomic_generator.py::format_generation_results()`

**优化 LLM 调用**：
- 查看 `textgraphtree/models/llm/batch_llm_wrapper.py`：批量请求优化
- 查看 `textgraphtree/utils/batch_request_manager.py`：批量请求管理

---

## 总结

TextGraphTree 的核心思想是：
...
希望这份文档能帮助你快速理解 TextGraphTree 的代码架构和核心逻辑！如有疑问，建议查看具体的代码实现和注释。
