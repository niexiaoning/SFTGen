...
from textgraphtree.bases import BaseLLMClient
from textgraphtree.models import (
    AggregatedGenerator,  # pragma: allowlist secret
    AtomicGenerator,
...
from textgraphtree.models.llm.batch_llm_wrapper import BatchLLMWrapper
from textgraphtree.templates import ATOMIC_ANSWER_PROMPT
from textgraphtree.utils import compute_content_hash, detect_main_language, logger, run_concurrent
...
async def generate_qas(
    ...
    if target_qa_pairs and batches:
        ...
        estimated_qa_per_batch = {
            ...
            "aggregated": 2.0,  # aggregated模式每个batch约2个QA对  # pragma: allowlist secret
    ...
    if mode == "all":
        ...
        generators = [
            ...
            (
                ...
                "aggregated",  # pragma: allowlist secret
            ),
            ...
            (TGTGenerator(actual_llm_client, data_format=data_format, default_dimension=generation_config.get("default_dimension", "concept_explanation")), "tgt"),
        ]
            ...
            if target_qa_pairs and gen_mode in mode_targets:
                ...
                if mode_target > 0:
                    ...
                    estimated_qa_per_batch_mode = {
                        ...
                        "aggregated": 1.5,  # aggregated模式每个batch约1-2个QA对  # // pragma: allowlist secret
        ...
        async def run_atomic_two_stage() -> list[dict[str, Any]]:
            ...
            async def answer_question(entry: dict[str, Any]) -> dict[str, Any]:
                try:
                    ...
                    if chinese_only:
                        from textgraphtree.templates import ATOMIC_ANSWER_PROMPT_CHINESE_ONLY
