...
from textgraphtree.bases import BaseGraphStorage, BaseKVStorage, BaseTokenizer
    ...
    if len(communities) == 0:
        ...
        if len(nodes) > 0:
            if method == "ece":
                ...
                max_tokens = method_params.get("max_tokens_per_community", 10240)  # pragma: allowlist secret
                total_tokens = sum(node[1].get("length", 100) for node in nodes)
                
                if total_tokens <= max_tokens:  # pragma: allowlist secret
                    # 所有节点合并为一个社区
