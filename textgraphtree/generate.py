...
def main():
    ...
    parser.add_argument(
        "--config_file",
        help="Config parameters for TextGraphTree.",
        default=files("textgraphtree").joinpath("configs", "aggregated_config.yaml"),  # pragma: allowlist secret
        type=str,
    )
    parser.add_argument(
        "--output_dir",
        help="Output directory for TextGraphTree.",
        default=sys_path,
        required=True,
        type=str,
    )
    ...
    logger.info(
        "TextGraphTree with unique ID %s logging to %s",
        unique_id,
    ...
    logger.info("TextGraphTree completed successfully. Data saved to %s", output_path)
