...
def main():
    ...
    parser.add_argument(
        ...
        default=files("textgraphtree").joinpath("configs", "evaluation_config.yaml"),
        type=str,
    )
    parser.add_argument(
        "--output_dir",
    ...
    # TGT 指标参数
    tgt_group = parser.add_argument_group("TGT 指标计算")
    tgt_group.add_argument("--tgt-taxonomy", help="TGT 意图树文件路径", required=False)
    tgt_group.add_argument("--tgt-results", help="TGT 结果文件路径", required=False)
    tgt_group.add_argument("--tgt-output", help="TGT 指标输出文件路径", required=False)
    ...
    graph_gen = TextGraphTree(unique_id=unique_id, working_dir=working_dir)
    ...
    # TGT 指标计算模式
    if hasattr(args, 'tgt_taxonomy'):
        success = run_tgt_metrics(args)
